    ...
    useEffect(() => {
        if (isOpen && task?.projectId) {
            const loadProjectMembers = async () => {
                setLoading(true);
                try {
                    const response = await apiClient.getProject(task.projectId);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const updatedTask = await apiClient.updateTask(task.id, {
                ...
                assigneeId: formData.assigneeId || null, // Allow unassigning
            ...
            <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg shadow-lg w-full max-w-md p-6 text-zinc-900 dark:text-white max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        <button type="button" onClick={() => setIsOpen(false)} className="rounded border border-zinc-300 dark:border-zinc-700 px-5 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 transition" >
                            Cancel
                        </button>
